...
function db_connect(){
	...
	$link=mysqli_connect($dblocalhost,$phome_db_username,$phome_db_password);
	//mysqli_select_db($phome_db_dbname);
	if(empty($link))
	{
...
function DoSetDbChar($dbchar){
	...
	if($dbchar&&$dbchar!='auto')
	{
		...
		@mysqli_query($link,'set character_set_connection='.$dbchar.',character_set_results='.$dbchar.',character_set_client=binary;');
	}
}
...
function Ebak_GetMysqlVerForDb(){
    global $link;
	$sql=mysqli_query($link,"select version() as version");
